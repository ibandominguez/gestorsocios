...
export interface MembersState {
  ...
  hasMemberNotPaidFor3Years: (member: Member) => boolean;
}
...
export const useMembersStore = create<MembersState>((set, get) => ({
  ...
  members: [
    {
      ...
      children: [{ name: "Josue Betancor Machín", dateOfBirth: "2010-03-16" }],
  ...
  hasMemberNotPaidFor3Years: (member: Member) => {
    const { last3Years } = get();
    return last3Years.every((year) => !member.yearPayments.includes(year));
  },
}));
